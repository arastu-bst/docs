...
sidebarTitle: "FAQ"
description: "Answers to the most common questions about nowrun: account setup, card verification, supported apps including Android and HTML5 web games, pricing, and how the agent workflow works."
---
...
<Accordion title="How do I find my token?">
  You do not need one for normal use. Your agent runs `nowrun login`, a browser tab opens, and you approve the device. Tokens are only needed for CI or headless machines, where you can copy one from the dashboard and use `nowrun init -t <token>`. Paste it directly into your AI agent when prompted, or pass it with `nowrun init -t <token>`.
</Accordion>
...
<Accordion title="What engines are supported?">
  For Android, any engine that produces an `.apk` file is supported: native Android, Unity, Godot, Unreal Engine, and others. The `.aab` (Android App Bundle) format is **not** supported, configure your build pipeline to output a plain `.apk` file.

  nowrun also supports **HTML5** web games, deployed as a `.zip` web bundle. Create the app with `nowrun app create --type h5`, then deploy the bundle with `nowrun deploy -f <bundle>.zip`.
</Accordion>
...
<Accordion title="Can I deploy without an AI agent?">
  Yes, the CLI works independently. Run `nowrun login` to authenticate, then `nowrun deploy` to push your build. That said, nowrun is designed and optimized for agent-driven workflows, so using an AI agent gives you the smoothest experience.
</Accordion>

...
description: "Diagnose and fix common problems with nowrun: authentication errors, CLI install failures, Android and HTML5 deploy issues, and agents that can't find the CLI."
---
...
<Accordion title="`nowrun login` fails or times out">
...
<Accordion title="`.aab` file — can't deploy">
  **Cause:** for Android, nowrun only supports `.apk` files. Android App Bundles (`.aab`) are not supported. (HTML5 games deploy as a `.zip` web bundle instead.)
...
<Accordion title="Agent says it doesn't know how to deploy / can't find the skill">
  **Cause:** The nowrun skill and MCP server are installed during `nowrun login`. If login was skipped or failed, the agent won't have access to the nowrun command surface.

  **Fix:** Run `nowrun login` again to re-install the skill and MCP server, then ask your agent to retry the deployment.
